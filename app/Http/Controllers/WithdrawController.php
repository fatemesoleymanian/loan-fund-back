...
class WithdrawController extends Controller
{
    ...
    public function create(DepositRequest $request){
        ...
        try {
            ...
            if($acc->have_sms) $sms = $this->sendSms($request->amount, $acc->member_name, $acc->balance, $acc->member->mobile_number,'withdraw');

            return response()->json([
                'withdraw'=>$withdraw,
                'msg' => $sms != null ? ' با موفقیت برداشت شد. پیامک ارسال شد':'با موفقیت برداشت شد.',
                'success' => true,
                'sms' => $sms
            ], 201);
    ...
    public function closure(DepositRequest $request){
        ...
        try {
            ...
            DB::commit();
            $sms = null;
            if($acc->have_sms) $sms = $this->sendSms($request->account_id,null, null, $acc->member->mobile_number,'closure');
            return response()->json([
                'account'=>$account,
                'msg' => $sms != null ? ' با موفقیت برداشت شد. پیامک ارسال شد':'با موفقیت برداشت شد.',
                'success' => true,
